...
public class LobbyServer {
	...
	private DataOutputStream output, output1 = null;
	private String name;
	...
	public LobbyServer(Socket s1, Socket s2, String name) {
		...
		ChatTh.start();
		/*
		 * 로비에 접속한 유저가 방을 생성하거나 입장에 대한 반복문이다. 이건 바뀔수 있지만 일단 생성하면 create 로비에서 나가면 exit 그외
		 * 다른것이 들어오면 방 입장으로 본다. 방 번호는 1부터 순차적으로 증가하고 생성하면 roomNumber의 숫자로 메인서버에
		 * roomList에 키는 방 번호 벨류는 룸 객체를 생성한다. 입장일경우 방의 정보(방 번호, 방이름?, 비번)을 받아서 그에 맞는 room
		 * 객체를 호출한다. 그리고 유저를 추가한다.
		 * 
		 * 위 과정이 끝나면 게임룸으로 입장하게 된다. 게임 룸에서는 룸객체안에 있는 유저들만 통신, 채팅이 가능하게된다.
		 * 
		 * 
		 */
		...
		while (true) {
			try {
				...
				if (str.equals("Create")) {
					...
					String roomList = "";
					roomList = MainServer.roomList.toString();
					...
					if (MainServer.roomList.get(rNumber).getRoomSize() <= 3
							&& MainServer.roomList.get(rNumber).getStart() == false) {
						MainServer.roomList.get("로비").RoomUserListDel(name);// 방에 유저 삭제
						MainServer.roomList.get(rNumber).RoomUserListAdd(name);
						if (input.readUTF().equals(MainServer.roomList.get(rNumber).getRoomPass())) {
